...
function EditPost() {
  ...
  const postID = useRef();
  postID.current = location.state.data.id;
  const originTitle = location.state.data.title;
  const originContent = location.state.data.contents;
  const originImg = location.state.data.imageUrl;
  const {
    handleSubmit,
    watch,
    register,
  ...
  title.current = watch("title", originTitle);
  ...
  const [imageSrc, setImageSrc] = React.useState(originImg);
  ...
  const onSubmit = async (data) => {
    ...
    await axios({
      method: "PUT",
      url: `${serverUrl}/posts/post/${postID.current}`,
      mode: "cors",
      headers: {
